    ...
    >
      <el-row>
        <el-col :span="24">
          ...
          <template v-if="applyType==='56009'">
            <el-form-item ref="fileUpload" label="相关工作方案" prop="workPlan" class="redStar">
              <el-upload
                :action="fileUploadUrl"
                :on-preview="handlePreview"
                :before-remove="beforeRemove"
                :data="{fieldCode: 'BAWJ', fieldName: '相关工作方案'}"
                :file-list="form.workPlan"
                :multiple="false"
                :limit="1"
                :before-upload="beforeUpload"
                :on-remove="(file,fileList)=>{return handleRemove (file,fileList,'workPlan')}"
                :on-success="(file,fileList)=>{return fileUploadSuccess (file,fileList,'workPlan')}"
                :on-error="fileUploadError"
              >
                <el-button size="small" type="primary">点击上传</el-button>
                <span slot="tip" class="el-upload__tip" style="font-size:12px">支持扩展名：doc / docx / pdf</span>
