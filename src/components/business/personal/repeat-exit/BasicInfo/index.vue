...
<script>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          save: {
            ...
            customsOut: [{max: 50, message: '长度最大 50 个字符', trigger: 'blur'}]
          },
          submit: {
            itemName: [
    ...
    methods: {
      ...
      initData() {
        // 查询国籍信息
        queryDictionary('Country').then(({data}) => this.options.nationality = data);
        // 查询证件类型
        queryDictionary('IDCardType').then(({data}) => this.options.certificateType = data);
        // 查询审核机构数据字典
