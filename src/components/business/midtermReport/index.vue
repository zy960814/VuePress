      ...
      <apply-form :data="titleData" @tab-click="itemClick">
        ...
        <template #body>
          ...
          <div class="scrollBox" :style="{height: contentHeight}">
            <basic-information class="hxl_item" ref="basicInformation" id="basicInformation"></basic-information>
          </div>
        </template>
      </apply-form>
    </el-card>
...
export default {
  ...
  data() {
    return {
      dialogVisible: false,
      // 切换按钮标题
      titleData: ['申请文件标题:考古发掘计划申请(一般事项)'],
      applyFormData: {},
      menuKey: 1
    }
  },
  ...
  methods: {
    ...
    // 保存或提交
    saveOrSubmit(type) {
      Promise.all([this.$refs.basicInformation.getFormData(type)]).then(val => {
