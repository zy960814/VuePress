      ...
      <apply-form :data="titleData" @tab-click="itemClick">
        ...
        <template #body>
          <div class="scrollBox" :style="{height: contentHeight}">
            ...
            <apply-form-item data="省级文物行政部门意见" class="hxl_item" v-if="status == 5 || status == 6 || status == 7">
              <provincial-bureau :status='status' :allData="allData" ref="provincialBureau"></provincial-bureau>
            </apply-form-item>
            <apply-form-item data="中国古迹遗址保护协会意见" class="hxl_item" v-if="status == 6 || status == 7">
              <china-association :status='status' :allData="allData"></china-association>
            </apply-form-item>
            <apply-form-item data="文保司文物处意见" class="hxl_item">
...
import ChinaAssociation from './ChinaAssociation'
import CulturalRelics from './CulturalRelics'
...
.box-card {
  ...
  /deep/ .box-card .el-card__body .center {
    padding: 0px;
  }
</style>
