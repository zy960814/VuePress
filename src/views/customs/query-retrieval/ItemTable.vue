...
<template>
  <div class="content">
    ...
    <table-opt-menu @item-click="itemClick" style="margin: 5px 0">
      ...
      <el-select
          v-model="queryParams.condition.isExhibition"
          @change="search(1)"
          size="mini"
          clearable
          placeholder="类型"
        ...
        <el-option label="展览" value="1"></el-option>
      </el-select>
      <el-select
          v-model="value"
          @change="search(1)"
          size="mini"
          clearable placeholder="单证类型"
          class="mr-10">
        <el-option
            v-for="item in options.testData"
            :key="item.value"
...
<script>
  ...
  import {findStationBizList} from '@/api/appraisal/query-retrieval'
  ...
  export default {
    ...
    data() {
      return {
        tableType: 'query_all_appraisal_item',
        options: {
          itemDic: [{
            label: '文物出境',
            value: '56020-a'
    ...
    methods: {
      ...
      search(currentPage) {
        ...
        findStationBizList(this.queryParams).then(({data}) => {
