...
<script>
  ...
  import {saveApplyCexhibition, listCulturalRelic} from '@/api/personal/schedule/56020/repeat-exit'
  import MyDialog from '@/components/commons/MyDialog'
  import BusinessGuide from '@/views/personal/schedule/56020/temp-entry/exhibition-apply/BusinessGuide'
  import {getApplyCeDetail, getBTempBizId} from '@/api/appraisal/repeat-exit'
  ...
  export default {
    ...
    mounted() {
      ...
      // 复出境展览事项查询
      getApplyCeDetail({itemId}).then(({data}) => {
        // 文件回显, 数据适配
      ...
      // 根据复出境事项ID查询临时进境事项ID
      getBTempBizId({itemId}).then(({data}) => this.x_itemId = data);
    },
    methods: {
      // 锚点点击事件
      itemClick(tab) {
