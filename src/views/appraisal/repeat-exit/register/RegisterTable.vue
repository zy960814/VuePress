...
<script>
  ...
  import {registerAll, branch} from '@/api/appraisal/temp-entry'
  ...
  import {queryApplyCList} from '@/api/appraisal/repeat-exit'
  ...
  export default {
    ...
    computed: {
      ...
      dialogHeight: function () {
        return this.$store.getters.screenHeight > 1000 ? 1000 : this.$store.getters.screenHeight;
      },
    ...
    methods: {
      ...
      search(currentPage) {
        ...
        queryApplyCList(this.queryParams).then(({data}) => {
      ...
      modify(row) {
        const path = row.isExhibition === '展览'
          ? `/appraisal/repeat-exit/register/modify/exhibition-apply?id=${row.id}`
          : `/appraisal/repeat-exit/register/modify/personal-carry-apply?id=${row.id}`;
