<template>
  <div class="details-edit">
    <el-button type="primary" @click="registerClick">
      {{ this.form.isRegister == null || this.form.isRegister === '0' ? '登记' : '取消登记' }}
    </el-button>
    <div class="underContent">
      <el-form ref="form" :model="form" label-width="120px" label-position="left">
        ...
        <picture-upload ref="imgUpload" :action="fileUploadUrl" style="height: 318px" :type="20" :max-file-size="2" :limit="6"/>
      </el-form>
      <el-form ref="regForm" :model="regForm" :rules="rules" label-width="120px" label-position="left">
        <div class="reg_opinion">
          <common-language @language="language" class="clear-float" space-width="78px"
                           style="line-height: 32px; margin-bottom: 10px"/>
          <el-form-item label="登记意见" prop="remark">
            <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="regForm.remark"></el-input>
          </el-form-item>
        </div>
      </el-form>
    </div>
...
<script>
  ...
  import {regCulturalRelic} from '@/api/appraisal/temp-entry'
  ...
  export default {
    ...
    methods: {
      ...
      // 常用语
      language(value) {
        this.regForm.remark += value;
      },
      // 登记点击事件
      registerClick() {
        const isRegister = this.form.isRegister == null || this.form.isRegister === '0' ? '1' : '0';
        regCulturalRelic({
          crId: this.form.id,
          isRegister
        }).then(() => {
          this.$message({
            message: isRegister === '0' ? '取消登记成功' : '登记成功',
            type: 'success'
          });
          this.form.isRegister = isRegister;
          getAncestorAttr(this, 'x_regNext')(this.form);
        });
      }
    }
  }
