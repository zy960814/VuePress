  ...
  <div class="relic-list">
    ...
    <el-table
      ...
      <el-table-column prop="crName" label="状态" min-width="65">
        <template slot-scope="scope">{{ scope.row.isApproval === '1' ? '已审批' : '未审批' }}</template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      tableRowClassName({row, rowIndex}) {
        let rowClass = row.isApproval === '1' ? 'success-row' : 'warning-row';
      ...
      spNext(row) {
        ...
        this.tableData[index].isApproval = row.isApproval;
