...
<script>
  ...
  import {queryApplyDList} from '@/api/appraisal/temp-exit'
  import {saveDynamicCol, queryDynamicCol} from '@/api/basic'
  import {getNoInspectionNum, batchInspection} from '@/api/appraisal/temp-exit'
  ...
  export default {
    ...
    methods: {
      ...
      search(currentPage) {
        ...
        queryApplyDList(this.queryParams).then(({data}) => {
      ...
      click_0(row) {
        const path = `/appraisal/temp-exit/inspection/deal-with?id=${row.id}`;
