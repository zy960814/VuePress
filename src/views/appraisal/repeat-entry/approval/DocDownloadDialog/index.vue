    ...
    methods: {
      ...
      downloadDoc(row) {
        ...
        if (['01', '03', '04'].indexOf(row.croCertificateCode) > -1) {
          // console.log(row);
        } else if('07'== row.croCertificateCode) {
          window.open(`${API_PREFIX.APPRAISAL}/certificate/printTempOutCriPDF/${this.$store.state._20.tempEntryItemId}`);
        } else {
      ...
      // OFD 文件下载
      downloadOFD() {
        window.open(`${API_PREFIX.APPRAISAL}/certificate/downloadTempOutCriOFD/${this.$store.state._20.tempEntryItemId}`);
      }
    }
  }
</script>
