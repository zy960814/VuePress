...
<script>
  ...
  import BasicInfo from '@/views/personal/schedule/56020/temp-entry/personal-carry-apply/BasicInfo'
  import CulturalRelicInfo from '@/components/business/CulturalRelicInfo'
  import {saveApplyBpersonal, listCulturalRelic} from '@/api/personal/schedule/56020/temp-entry'
  import MyDialog from '@/components/commons/MyDialog'
  import BusinessGuide from '@/views/personal/schedule/56020/temp-exit/exhibition-apply/BusinessGuide'
  import {getApplyBpDetail} from '@/api/appraisal/temp-entry'
  ...
  export default {
    ...
    mounted() {
      ...
      getApplyBpDetail({itemId}).then(({data}) => {
          ...
          if (this.$store.state._20.tempEntryItemId != null) {
            ...
            listCulturalRelic(this.queryParams).then(result => {
              if (result.data.pagination.totalNum > 0) {
                ...
                saveApplyBpersonal(data).then(({data}) => {
                  // 保存事项ID
