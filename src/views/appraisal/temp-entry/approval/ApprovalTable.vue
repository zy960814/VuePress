...
<script>
  ...
  import {queryApplyBList, againBranch, tieUp} from '@/api/appraisal/temp-entry'
    ...
    methods: {
      ...
      handle(row) {
        const path = row.isExhibition === '展览'
          ? `/appraisal/temp-entry/approval/deal-with?id=${row.id}`
          : `/appraisal/temp-entry/approval/deal-with?id=${row.id}&type=personal`;
