...
<script>
  ...
  import BasicInfo from '@/components/business/appraisal/temp-entry/BasicInfo'
  ...
  import {getApplyBeDetail, handleAppointment, getApplyBpDetail} from '@/api/appraisal/temp-entry'
    ...
    mounted() {
      ...
      switch (itemType) {
        case 'exhibition':  // 临时进境展览事项查询
          ...
          getApplyBeDetail({itemId: this.$route.query.id}).then(({data}) => {
        ...
        case 'personal':  // 临时进境个人携带查询
          ...
          getApplyBpDetail({itemId: this.$route.query.id}).then(({data}) => {
            this.basicInfoData = data;
          });
          break
      }
          ...
          this.$refs['form'].validate(valid => {
            if (valid) {
              handleAppointment(this.form).then(() => {
                ...
                this.$router.push('/appraisal/temp-entry');
              });
            }
          });
        });
