...
<script>
  ...
  import {queryApplyBList} from '@/api/appraisal/temp-entry'
    ...
    methods: {
      ...
      click_0(row) {
        const path = row.isExhibition === '展览'
          ? `/appraisal/temp-entry/appointment/deal-with?id=${row.id}`
          : `/appraisal/temp-entry/appointment/deal-with?id=${row.id}&type=personal`;
